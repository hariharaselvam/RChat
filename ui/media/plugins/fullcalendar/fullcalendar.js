	...
	var position = el.css('position'),
		scrollParent = el.parents().filter(function() {
			...
			return (/(auto|scroll)/).test(
				parent.css('overflow') + parent.css('overflow-ui') + parent.css('overflow-x')
			);
		}).eq(0);
...
newMomentProto.stripTime = function() {
	...
	if (!this._ambigTime) {
		...
		a = this.toArray(); // array of ui/m/d/h/m/s/ms
					...
					for (i = 0; i < colSegs.length; i++) {
						...
						containerEl.append(
							seg.el.css({
								...
								bottom: -this.computeDateTop(seg.end, dayDate) // the ui position of the bottom edge
							})
						);
					}
				}
